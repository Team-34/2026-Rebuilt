...
public class FireControl extends SubsystemBase {
  ...
  public FireControl(Vision vision) {
    ...
    config.launcherOffsetX = 0.0; // how far forward the launcher is from robot center (m) -> Modified
    config.launcherOffsetY = 0.0; // how far left, 0 if centered -> Modified
  ...
  public void periodic() {
    ...
    ShotCalculator.ShotInputs inputs = new ShotCalculator.ShotInputs(
      ...
      ChassisSpeeds.fromFieldRelativeSpeeds(commandSwerveDrivetrain.getState().Speeds,
        commandSwerveDrivetrain.getState().RawHeading),
      ChassisSpeeds.fromRobotRelativeSpeeds(commandSwerveDrivetrain.getState().Speeds,
        commandSwerveDrivetrain.getState().RawHeading),
      hubCenter, 
      hubForward, 
      0.9, // vision confidence, 0 to 1
      commandSwerveDrivetrain.getPigeon2().getPitch().getValueAsDouble(), // pitch for tilt gate (0.0 if no gyro)
      commandSwerveDrivetrain.getPigeon2().getRoll().getValueAsDouble() // roll for tilt gate (0.0 if no gyro)
    );
